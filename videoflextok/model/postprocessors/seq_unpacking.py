...
__all__ = ["SequenceUnpacker", "SequenceInterleavedUnpacker"]
...
class SequenceInterleavedUnpacker(SequenceUnpacker):
    """
    Module for unpacking interleaved sequences from nested packed formats, designed to manage hierarchical
    tensor unpacking within a dictionary-based pipeline. It assumes each original sequence is first packed and then interleaved along the first time dimension.
    Works with BlockWiseSequenceInterleavePacker
    ...
    def forward(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        inner_seqs_unpacked = []
        for inner_seq_packed, inner_ps in zip(inner_seqs_packed, inner_packed_shapes):
            # Reshape the interleaved packed sequences
            tokens_per_time = sum([ps.numel() for ps in inner_ps])
            inner_seq_packed = einops.rearrange(
                inner_seq_packed, "b (t l) d -> b t l d", l=tokens_per_time
            )
            # Unpack inner sequences
            inner_seq_unpacked = einops.unpack(inner_seq_packed, inner_ps, "b t * d")
            inner_seqs_unpacked.append(inner_seq_unpacked)
