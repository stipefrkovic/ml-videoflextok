...
class Registers1D(nn.Module):
    ...
    # TODO: Add option to add padding tokens instead of truncating sequences
...
class RegistersTemporal2D(Registers1D):
    """
    2D register token module. Similar to Registers1D, but creates 2D register tokens [T, N], where 1st dimension is time.
    The size sampling is done over the second dimension, while the first dimension stays intact, the same as the input time dimension.

    TODO: add a factorized time-depth version.
    ...
    Args:
        ...
        registers_write_key: Key to write the generated registers into the output dictionary.
        dim: Dimension of each register token.
        t_max: Height of the 2D register tokens.
        n_min: Minimum width (second dimension) of register tokens to sample.
        n_max: Maximum width (second dimension) of register tokens available.
        n_eval: Optional fixed width of register tokens to use during evaluation; defaults to n_max if not specified.
        size_sampling_mode: Method to sample the width of register tokens; choices include 'uniform', 'powers_of_two',
            or specific sizes with 'k={sizes}' (e.g., 'k=1-2-4-8').
        ordering_mode: Method to select and order register tokens; options are 'nested' or 'avg_pool'.
        extra_registers_write_key: Optional key to write extra registers into the output dictionary.
        extra_registers_dim: Dimension of the extra registers if extra_registers_write_key is specified.
    """
    ...
    def __init__(
        ...
        super().__init__(
            input_tensor_list_read_key=input_tensor_list_read_key,
            register_sizes_read_write_key=register_sizes_read_write_key,
            registers_write_key=registers_write_key,
            dim=dim,
            n_min=n_min,
            n_max=n_max,
            n_eval=n_eval,
            size_sampling_mode=size_sampling_mode,
            ordering_mode=ordering_mode,
            extra_registers_write_key=extra_registers_write_key,
            extra_registers_dim=extra_registers_dim,
        )
        self.t_max = t_max

        # Override the 1D registers with 2D registers
        self.registers = nn.Parameter(torch.randn(t_max, n_max, dim), requires_grad=True)
        trunc_normal_(self.registers, std=0.02)

        # Override extra registers if needed
        if extra_registers_write_key is not None:
            del self.extra_registers  # Delete the 1D extra register parameter
            self.extra_registers = nn.Parameter(
                torch.randn(t_max, n_max, extra_registers_dim), requires_grad=True
            )
            trunc_normal_(self.extra_registers, std=0.02)
    ...
    def get_registers(self, register_sizes_list, batch_sizes_list, temp_sizes_list, registers):
        registers_list = []
        for size, batch_size, temp_size in zip(
            register_sizes_list, batch_sizes_list, temp_sizes_list
        ):
            if self.ordering_mode == "nested":
                reg = registers[:temp_size, :size]
            else:
                raise NotImplementedError(f"Ordering mode {self.ordering_mode} not implemented.")

            registers_list.append(einops.repeat(reg, "t n d -> b t n d", b=batch_size))
        return registers_list

    @torch.compiler.disable
    def forward(self, data_dict: Dict[str, Any]) -> Dict[str, Any]:
        # List of [B, T, n1, n2, ..., nN, D] tensors
        x_list = data_dict[self.input_tensor_list_read_key]
        batch_sizes = [x.shape[0] for x in x_list]
        temp_sizes = [x.shape[1] for x in x_list]
        ...
        data_dict[self.registers_write_key] = self.get_registers(
            register_sizes_list, batch_sizes, temp_sizes, self.registers
        )
        if self.extra_registers_write_key is not None:
            data_dict[self.extra_registers_write_key] = self.get_registers(
                register_sizes_list, batch_sizes, temp_sizes, self.extra_registers
            )
