...
def make_attn(in_channels, norm_type="groupnorm"):
    return AttnBlockWrapper(in_channels, norm_type=norm_type)
...
class AttnBlockWrapper(AttnBlock):
    def __init__(self, in_channels, norm_type="groupnorm"):
        super().__init__(in_channels, norm_type=norm_type)
...
class TimeUpsampleResCausal2x(nn.Module):
    ...
    def forward(self, x):
        alpha = torch.sigmoid(self.mix_factor)
        x = torch.nn.functional.interpolate(
            x.to(torch.float32), scale_factor=[2.0, 1.0, 1.0], mode="nearest"
        ).to(x.dtype)
        x_ = self.conv(x)
...
class EncoderCausal3D(nn.Module):
    def __init__(
        ...
        **ignore_kwargs,
        ...
        super().__init__()
        ignore_kwargs.pop("use_checkpoint", None)
        self.ch = ch
        self.temb_ch = 0
        ...
        self.spatial_ds = (
            list(range(0, self.num_resolutions - 1)) if spatial_ds is None else spatial_ds
        )
        self.tempo_ds = (
            [self.num_resolutions - 2, self.num_resolutions - 3] if tempo_ds is None else tempo_ds
        )
...
class DecoderCausal3D(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        super().__init__()
        ignorekwargs.pop("use_checkpoint", None)
